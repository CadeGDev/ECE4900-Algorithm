...
class InferenceTester:
    """
    A class to handle the loading of a neural network model and perform inference on a given image.
    ...
    Methods:
        ...
        perform_inference(): Processes an image and performs inference using the loaded model.
    """
...
def evaluate_model_accuracy(model_path, csv_file_path, base_image_folder, num_samples=None):
    ...
    """
    tester = InferenceAccuracyTester(model_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', action="store", required = True, help = "Pre-trained model path")
    parser.add_argument('--images', action="store", required = True, help = "Folder containing images to be processed by script")
    args = parser.parse_args()
